        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_310px]">
          ...
          <section className="rounded-2xl border border-neutral-200/50 bg-white shadow-premium-sm overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-200/60 px-6 py-5 bg-neutral-50/20">
              <div>
                <h2 className="text-base font-semibold text-neutral-850">Team Roster</h2>
              ...
              {members.map((member) => (
                <div key={member.id} className="p-5 space-y-4 bg-white">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8.5 w-8.5 items-center justify-center rounded-full bg-gradient-to-br from-neutral-50 to-neutral-100 font-semibold text-neutral-650 text-xs shrink-0 border border-neutral-200 shadow-premium-sm">
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-neutral-50/50 text-[10px] font-semibold uppercase tracking-widest text-neutral-455 border-b border-neutral-100/60">
                  <tr>
                    ...
                    <th className="px-4 py-4.5">Leave Summary</th>
...
function Panel({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-neutral-200/50 bg-white p-5 shadow-premium-sm">
      <h2 className="mb-4 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-widest text-neutral-455">
...
function RequestLine({ request }: { request: LeaveRequest }) {
  return (
    <div className="rounded-xl bg-neutral-50/50 border border-neutral-100 p-3.5 text-xs shadow-premium-sm">
      <div className="font-semibold text-neutral-850">
        {request.user?.name} · <span className="text-emerald-750">{request.leave_type.name}</span>
      </div>
      <div className="text-[11px] text-neutral-500 mt-2.5 flex flex-wrap items-center gap-1 font-medium">
        <span className="font-semibold">{formatShortDate(request.starts_at)} – {formatShortDate(request.ends_at)}</span>
        <span>·</span>
        <span className="font-semibold text-neutral-700">{formatDays(request.requested_days)} day(s)</span>
        <span>·</span>
        <span className="rounded-full bg-neutral-100 border border-neutral-200 px-1.5 py-0.2 font-semibold text-neutral-600 uppercase text-[9px]">{request.status}</span>
      </div>
    </div>
  );
}
