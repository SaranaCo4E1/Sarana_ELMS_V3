            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-t-neutral-100/60">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {form.attachments.map((file, index) => (
                  <span key={index} className="inline-flex max-w-full items-start gap-1.5 rounded-lg border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-sm font-normal text-neutral-600 animate-fade-in">
                    <span className="wrap-anywhere min-w-0">{file.name}</span>
                    <button type="button" onClick={() => removeAttachment(index)} className="shrink-0 text-neutral-400 hover:text-neutral-600 transition-colors">
                      <X size={11} />
                    </button>
                  </span>
                ))}
        ...
        {requests.map((request) => (
          <div key={request.id} className="p-5 space-y-4">
            ...
            <div className="flex justify-end gap-3.5 pt-1">
              <button
                className="text-sm font-normal text-neutral-505 hover:text-neutral-800 transition-all active:scale-95 cursor-pointer"
            ...
            <div className="border-t border-neutral-100 px-6 py-4 bg-neutral-50/20 flex justify-between items-center">
              <div>
                {viewDetailsRequest.status === 'pending' && (
                  <button
                    className="inline-flex items-center justify-center rounded-lg border border-rose-250 bg-rose-50 px-5 py-2.5 text-sm font-semibold uppercase tracking-wide text-rose-700 transition-all hover:bg-rose-100 cursor-pointer"
