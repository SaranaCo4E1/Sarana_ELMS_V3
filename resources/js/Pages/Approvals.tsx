      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="rounded-2xl border border-neutral-200/50 bg-white shadow-premium-sm overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-200/60 px-6 py-5 bg-neutral-50/20">
            <div>
              <h2 className="text-base font-semibold text-neutral-850">Pending Approvals</h2>
              <p className="text-xs font-medium text-neutral-450 mt-1">Review time off applications and submit your decision</p>
            </div>
            <div className="flex flex-wrap gap-2.5">
              <div className="relative">
                <Search className="absolute left-3 top-2.5 text-neutral-400" size={13} />
                <input
                  className="w-44 rounded-xl border border-neutral-200/70 bg-white py-1.5 pl-8 pr-3 text-xs text-neutral-700 placeholder-neutral-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
              ...
              <select
                className="rounded-xl border border-neutral-200/70 px-3 py-1.5 text-xs bg-white font-semibold text-neutral-600 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
                value={department}
              ...
              <div key={request.id} className="grid gap-6 p-6 lg:grid-cols-[1fr_320px]">
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm font-semibold text-neutral-850">{request.user?.name}</span>
                    <span className="inline-flex items-center rounded-full bg-neutral-100 px-2.5 py-0.5 text-[11px] font-semibold text-neutral-650 border border-neutral-200/50">
                      {request.user?.department?.name ?? 'General'}
                    </span>
                    <span className="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-750 border border-emerald-100/70">
                      {request.leave_type.name}
                    </span>
                  </div>

                  <div className="text-xs text-neutral-500 font-medium">
                    Requested: <span className="font-semibold text-neutral-800">{formatShortDate(request.starts_at)}</span> to <span className="font-semibold text-neutral-800">{formatShortDate(request.ends_at)}</span>
                    <span className="mx-2">·</span>
                    <span className="font-semibold text-emerald-700">{formatDays(request.requested_days)} working day(s)</span>
                  </div>

                  {request.reason ? (
                    <p className="text-sm text-neutral-650 leading-relaxed bg-neutral-50/20 border border-neutral-200/60 rounded-xl p-3.5 italic shadow-premium-sm max-w-2xl">
                      "{request.reason}"
                    </p>
                  ) : (
                  ...
                  <div className="flex flex-wrap gap-2 pt-1.5">
                    {(request.attachments ?? []).map((attachment) => (
                      <span
                        key={attachment.id}
                        className="inline-flex items-center gap-1.5 rounded-full bg-white border border-neutral-200 px-3 py-1 text-xs font-semibold text-neutral-600 transition-all hover:bg-neutral-50 shadow-premium-sm"
                      >
                    ...
                    {(request.attachments ?? []).length === 0 && (
                      <span className="text-xs font-semibold text-neutral-450">No attachments provided</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between rounded-2xl border border-neutral-200/50 bg-white p-4.5 shadow-premium-sm">
                  <div>
                    <label className="block text-[10px] font-semibold uppercase tracking-widest text-neutral-450 mb-1.5">
                      Decision Comment
                    </label>
                    <textarea
                      className="h-24 w-full rounded-xl border border-neutral-200/70 p-3 text-xs text-neutral-700 placeholder:text-neutral-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none resize-none"
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    <button
                      className="flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 py-2.5 text-xs font-semibold text-white transition-all shadow-md shadow-emerald-600/10 hover:from-emerald-700 hover:to-teal-600 hover:-translate-y-0.5 active:translate-y-0 active:scale-97 cursor-pointer"
                      onClick={() => decide(request.id, 'approved')}
                    >
                      <Check size={14} /> Approve
                    </button>
                    <button
                      className="flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-rose-600 to-red-500 py-2.5 text-xs font-semibold text-white transition-all shadow-md shadow-rose-600/10 hover:from-rose-700 hover:to-red-600 hover:-translate-y-0.5 active:translate-y-0 active:scale-97 cursor-pointer"
        ...
        <div className="rounded-2xl border border-neutral-200/50 bg-white shadow-premium-sm overflow-hidden">
          <div className="border-b border-neutral-200/60 px-6 py-5 bg-neutral-50/20">
            <h2 className="text-base font-semibold text-neutral-850">Recent Decisions</h2>
            <p className="text-xs font-medium text-neutral-450 mt-1">Log of recently approved or rejected leave requests</p>
          </div>
              ...
              return (
                <div key={request.id} className="p-5 space-y-4 bg-white">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-neutral-850 text-xs">{request.user?.name}</span>
                    <span className={`inline-flex items-center rounded-full border ${statusStyle.border} ${statusStyle.bg} ${statusStyle.text} px-2.5 py-0.5 text-[11px] font-semibold tracking-wide`}>
                      {request.status.toUpperCase()}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-xs text-neutral-550">
                    <div>
                      <div className="text-[10px] text-neutral-400 uppercase font-semibold tracking-wider">Leave Type</div>
                      <div className="font-semibold text-neutral-700 mt-0.5">{request.leave_type.name}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-neutral-400 uppercase font-semibold tracking-wider">Days</div>
                      <div className="font-semibold text-neutral-700 mt-0.5">{formatDays(request.requested_days)}</div>
                    </div>
                  </div>

                  <div className="text-xs text-neutral-500 font-medium">
                    Dates: <span className="font-semibold text-neutral-800">{formatShortDate(request.starts_at)} – {formatShortDate(request.ends_at)}</span>
                  </div>

                  {request.manager_comment && (
                    <div className="bg-neutral-50/50 rounded-xl p-3 border border-neutral-100 text-xs text-neutral-500 font-medium">
                ...
                {recentDecisions.map((request) => {
                  ...
                  return (
                    <tr key={request.id} className="transition-all hover:bg-neutral-50/40">
                      <td className="px-6 py-4 font-semibold text-neutral-850">{request.user?.name}</td>
                      <td className="px-4 py-4 text-neutral-600 font-semibold">{request.leave_type.name}</td>
                      ...
                      <td className="px-4 py-4 font-semibold text-neutral-700">{formatDays(request.requested_days)}</td>
                      <td className="px-4 py-4">
                        <span className={`inline-flex items-center rounded-full border ${statusStyle.border} ${statusStyle.bg} ${statusStyle.text} px-2.5 py-0.5 text-[11px] font-semibold tracking-wide`}>
                          {request.status.toUpperCase()}
                        </span>
                      </td>
                      <td className="px-6 py-4 max-w-80 truncate text-neutral-550 font-medium" title={request.manager_comment ?? ''}>
                        {request.manager_comment ?? '–'}
                      </td>
                    </tr>
                  );
...
function Metric({ label, value, icon, variant }: { label: string; value: string | number; icon: React.ReactNode; variant: 'amber' | 'emerald' | 'rose' | 'indigo' }) {
  ...
  return (
    <div className={`rounded-2xl border ${theme.border} p-6 bg-white shadow-premium-sm hover:shadow-premium-md transition-all duration-300 relative overflow-hidden group`}>
      ...
      <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-widest text-neutral-450 relative z-10">
        <span>{label}</span>
